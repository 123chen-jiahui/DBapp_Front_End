      ...
      <div class="loginbox">
        ...
        <div class="loginbox-in">
          ...
          <div style="text-align:center">
            <button type="primary" class="register_btn" @click="patient = !patient">
              {{ patient ? '病人注册' : '员工注册' }}
            </button>
          </div>
          <div class="userbox">
            ...
            <input v-model="ID" class="user" placeholder="身份证号">
          </div>
          <div class="pwdbox">
            <v-icon class="iconfont">
              mdi-account
            </v-icon>
            <input v-model="name" class="user" placeholder="姓名">
          </div>
          <div class="pwdbox">
            <v-icon class="iconfont">
              mdi-account
            </v-icon>
            <input v-model="gender" type="radio" value="1"><label class="gend">男</label>
            <input v-model="gender" type="radio" value="0"><label class="gend">女</label>
          </div>
          <div class="pwdbox">
            <v-icon class="iconfont">
              mdi-account
            </v-icon>
            <input v-model="birth" class="user" placeholder="生日(1900-01-01)">
          </div>
          <div class="pwdbox">
            <v-icon class="iconfont">
              mdi-account
            </v-icon>
            <input v-model="phone" class="user" placeholder="电话号码">
          </div>
          <div v-if="!patient" class="pwdbox">
            <v-icon class="iconfont">
              mdi-account
            </v-icon>
            <input v-model="role" class="user" placeholder="职务(0-2)">
          </div>
          <div v-if="!patient" class="pwdbox">
            <v-icon class="iconfont">
              mdi-account
            </v-icon>
            <input v-model="depart" class="user" placeholder="部门(1-10)">
          </div>
          <div v-if="!patient" class="pwdbox">
            <v-icon class="iconfont">
              mdi-account
            </v-icon>
            <input v-model="add" class="user" placeholder="地址">
          </div>
          <div class="pwdbox">
            <v-icon class="iconfont">
              mdi-key
          ...
          <div class="pwdbox">
            ...
            <input v-model="conpwd" class="pwd" type="password" placeholder="确认密码">
          </div>
          <div style="text-align:center">
            <!-- <button
              type="primary"
            ...
            <el-button type="primary" class="register_btn" @click="register" :loading="loading_register">
              注册账号
            </el-button>
          </div>
        </div>
      ...
      if (this.patient) {
        ...
        axios.post('/auth/register_patient', {
          ...
          .then(function (response) {
            outerthis.loading_register = false
            let alt = '注册成功！以下是您的账号，请务必牢记：\n'
